...
import argparse
import os
import tools.evaluation.CodeBLEU.bleu as bleu
import tools.evaluation.CodeBLEU.weighted_ngram_match as weighted_ngram_match
import tools.evaluation.CodeBLEU.syntax_match as syntax_match
import tools.evaluation.CodeBLEU.dataflow_match as dataflow_match
...
def compute_codebleu(hypothesis, references, lang, params='0.25,0.25,0.25,0.25'):
    ...
    kw_file = os.path.join(root_directory, f"evaluation/CodeBLEU/keywords/{lang}.txt")
...
def code_bleu(preds, golds, lang, prefix=None):
    code_bleu_score, cpl_data = compute_codebleu(hypothesis=preds, references=golds, lang=lang)
    return {f"{prefix}_codebleu" if prefix else "codebleu": code_bleu_score * 100}, cpl_data
